<?php
/**
 * Checkout Settings REST API Controller.
 *
...
use WP_Error;
use QuickCartShopping\Helper\Sanitization\CheckoutDataSanitization;
 ...
 * Checkout Settings REST API Controller
 */
class CheckoutData extends WP_REST_Controller
{
    /**
     * Constructor.
     */
    public function __construct() {
        ...
        $this->rest_base = 'save-checkout-settings';
    }

    /**
     * Register the routes for Checkout Settings.
     *
     * @return void
     */
    public function register_routes()
    {
        ...
        register_rest_route(
            $this->namespace,
            '/update-checkout-settings/(?P<id>[a-zA-Z0-9_-]+)',
            [
                [
    ...
    public function save_form_data( WP_REST_Request $request )
    {
        ...
        $sanitized_data = CheckoutDataSanitization::sanitize_checkout_settings( $params );

        $saved = update_option( 'quick_cart_checkout_settings', $sanitized_data );
    ...
    {
        $raw = get_option( 'quick_cart_checkout_settings', [] );
    ...
    public function update_form_data( WP_REST_Request $request )
    {
        ...
        $existing = get_option( 'quick_cart_checkout_settings', [] );
        ...
        $sanitized_data = CheckoutDataSanitization::sanitize_checkout_settings( $merged );

        $updated = update_option( 'quick_cart_checkout_settings', $sanitized_data );
